...
//	}
	...
	public CHESSDiagramsGeneratorService() {
		super();
	}
	...
	public DiagramDescriptor createDiagramWithDescriptor(Diagram diagram, Shell shell, IProgressMonitor monitor) {
		...
		try {
			if (saveFilePath.endsWith("svg")) {
				...
				copyImageUtil.copyToImage(diagram, imagePath, ImageFileFormat.PNG, new NullProgressMonitor(), PreferencesHint.USE_DEFAULTS);
			}else if  (saveFilePath.endsWith("jpg")) {
				copyImageUtil.copyToImage(diagram, imagePath, ImageFileFormat.JPG, new NullProgressMonitor(), PreferencesHint.USE_DEFAULTS);
			}
