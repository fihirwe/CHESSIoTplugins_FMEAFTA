...
public class Port extends NamedElement {
	 ...
	 * A copy of the failure will be created and added to handle EMF containment relationship.
	 * @param failure
	 * @param rule is not null in caso of intra component propagation //todo what if we consider FPTC for connections?
	 * @return <code>true</code> if the failure was added to the set of new failures.
	 */
	public boolean addFailure(Failure failure, Failure previousFailure,Rule rule) {
		if (Failure.doCollectionContainFailure(getFailures(), failure)) {
			...
			if (rule != null)
				existingFailure.getRaisedByRule().add(rule);
			return false;
		...
		Failure f = new Failure(failure, previousFailure);
		if (rule != null)
			f.getRaisedByRule().add(rule);
		return getNewFailures().add(f);
	}
	 ...
	 * @return <code>true</code> if the failure was added to the set of new failures.
	 */
	public boolean addFailure(Failure failure, Collection<Failure> previousFailures, Rule rule) {
		if (Failure.doCollectionContainFailure(getFailures(), failure)) {
			Failure existingFailure = Failure.getFailureInCollection(getFailures(), failure);
			existingFailure.addPreviousFailures(previousFailures);
			if (!existingFailure.getRaisedByRule().contains(rule)){
				existingFailure.getRaisedByRule().add(rule);
			}
			return false;
		} else if (Failure.doCollectionContainFailure(getNewFailures(), failure)) {
		...
		Failure f = new Failure(failure, previousFailures);
		f.getRaisedByRule().add(rule);
		return getNewFailures().add(f);
	}
	...
	public Object getPreviousFailures() {
