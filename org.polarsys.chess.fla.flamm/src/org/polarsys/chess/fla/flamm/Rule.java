...
public class Rule extends MinimalEObjectImpl.Container implements EObject {
	...
	public void fire(Collection<Failure> previousFailures) {
		for (Expression expression : getOutputExpression()) {
			...
			for (Failure failure : expression.getFailures()) {
				if (failure.getType() == FailureType.VARIABLE) {
					port.addFailure(variableAssignments.get(failure.getId()), previousFailures, this);
				} else {
					port.addFailure(failure, previousFailures, this);
				}
			}
		}
	}
	...
	/**
	 * 
	 * @return true if the rule is of kind "x.noFailure,..., z.noFailure -> ...."
	 */
	public boolean isSourceRule (){
		
		for (Expression exp : this.getInputExpression()){
			for (Failure fail: exp.getFailures()){
				if (!fail.getType().equals(FailureType.NO_FAILURE))
					return false;
			}
		}
		return true;
