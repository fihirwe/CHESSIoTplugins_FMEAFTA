...
public class ContractImplementationCommand extends AbstractJobCommand {
	...
	public void execPreJobOperations(ExecutionEvent event, IProgressMonitor monitor) throws Exception {
		...
		if ((instantiatedArchitecures != null) && (!instantiatedArchitecures.isEmpty())) {
			...
			if (dialog.goAhead()) {
				...
				resultFilePath = ocraDirectoryUtil.getCommandCheckImplementationResultPath(configurationName);
			}
		}
	...
	public void execJobCommand(ExecutionEvent event, IProgressMonitor monitor) throws Exception {
		...
		String smvOutputPath = smvExportService.exportSingleSmv(umlSelectedComponent, showPopups, smvFileDirectory,
				monitor);
		commandExecuted = ocraExecService.executeCheckContractImplementation(umlSelectedComponent,
				selectedInstantiatedArchitectureConfiguration, umlSelectedResource, smvOutputPath, isDiscreteTime,
				usexTextValidation, showPopups, ossFilepath, resultFilePath, true, monitor);
	...
	public void execPostJobOperations(ExecutionEvent event, NullProgressMonitor nullProgressMonitor) throws Exception {
		...
		if (commandExecuted) {
			...
			// Store the result
			analysisResultUtil.createOrUpdateAnalysisContext(Analysis.CONTRACT_IMPLEMENTATION_ANALYSIS, ECollections.emptyEList(),
					resultFilePath, false, umlSelectedComponent, selectedInstantiatedArchitectureConfiguration, null);

			// Visualize the result
			analysisResultUtil.showResult(CheckContractImplementation.FUNCTION_NAME, resultFilePath);
		}
